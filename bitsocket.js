    ...
    if(event.type == 'mempool') {
        ...
        for (var i = 0; i < event.data[0].out.length; i++) {
            ...
            if(event.data[0].out[i] && event.data[0].out[i].e && event.data[0].out[i].e.a) {
                ...
                ch.innerHTML = '<b><a href="https://explorer.bitcoin.com/bch/tx/' + event.data[0].tx.h + '" target="_blank">*</a> ' + event.data[0].out[i].e.a.substring(0, 4) + '...'  + event.data[0].out[i].e.a.substring(34) + '</b> received <b>' + (event.data[0].out[i].e.v * 0.00000001).toFixed(8) + ' BCH</b>';
